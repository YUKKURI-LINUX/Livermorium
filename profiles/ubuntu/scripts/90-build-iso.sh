...
# The previous version of commands is here:
...
# The current version of commands (comparing with the previous version, it added exclusions)
mksquashfs "${CHROOT_DIR}" "${CASPER_DIR}/filesystem.squashfs" \
...
(
  cd "${ISO_DIR}"
  # Find all files and calculate their checksums (except `md5sum.txt` itself)
  find . -type f ! -name "md5sum.txt" -exec md5sum {} + > md5sum.txt
)

OUT_ISO="${OUT_DIR}/${BASENAME}-$(date +%Y%m%d_%H%M).iso"
echo "[$SCRIPT_NAME] Creating ISO... -> ${OUT_ISO}"
